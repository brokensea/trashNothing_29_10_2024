...
export default function Marktplatz() {
    ...
    const handleSearch = async () => {
        ...
        if (searchTerm && selectedKategorie.length > 0) {
            url += `?title=${encodeURIComponent(searchTerm)}&kategorie=${encodeURIComponent(selectedKategorie.join(','))}`;
        } else if (searchTerm) {
            url += `/searchByTitle?title=${encodeURIComponent(searchTerm)}`;
        } else if (selectedKategorie.length > 0) {
            url += `/searchByKategorie?kategorie=${encodeURIComponent(selectedKategorie.join(','))}`;
        }
        ...
        <>
            <section className="background_section_m">
                ...
                <div className="search_area">
                    ...
                    />
                    <button className="post_button" onClick={handleSearch}>Produkt einstellen</button>
                </div>
            </section>
